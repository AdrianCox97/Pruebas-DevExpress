 ...
 * Author :       Adrian Cox
 *
 * Date Created : Tuesday, July 11, 2017 16:46:18
...
CREATE TABLE ALUMNOS(
    ...
    Status              bit              NOT NULL,
    IDUsuario_Cambio    nvarchar(max)    NULL,
...
VALUES (@Matricula, @NombreAlumno, @ApellidoPaterno, @ApellidoMaterno, @Correo, @Contrasenia, @Telefono, @Status, @IDUsuario_Cambio, @IDCarrera, @IDGrupo);
...
SET
	...
	Telefono = @Telefono,
	Status = @Status,
	IDUsuario_Cambio = @IDUsuario_Cambio,
	IDCarrera = @IDCarrera,
	IDGrupo = @IDGrupo
WHERE
	Matricula = @Matricula;
...
VALUES (@NombreCarrera, @Status, @IDUsuario_Cambio, @IDDivision);
...
VALUES (@NombreDivision, @Status, @IDUsuario_Cambio);
...
VALUES (@Cuatrimestre, @Grupo, @Status, @IDUsuario_Cambio);
...
DECLARE
	...
	SELECT @IDUSUARIO = IDUsuario_Cambio from #ins
	SELECT @IDUSUARIO = IDUsuario_Cambio from #del
